...
- 已有未完結的有效進度時，大按鈕顯示 **「繼續遊戲」**。
- 最後一輪已抵達結局時，大按鈕回到 **「開始遊戲」**，從故事起點開新一輪；已解鎖回憶、CG、結局與歷史 frontier 保留。
- 玩家明確從結局後按「開始遊戲」開新一輪時，這一輪的 cursor 可由標題「繼續遊戲」恢復；一般 Memories replay 仍不可把 Continue 從歷史 frontier 拉回舊事件。
- 「回憶」與「CG」是兩個同級的小按鈕。
- 聲音／設定不再占用第四個主按鈕位置；放到角落 icon / HUD。
- 不提供獨立「重新開始」按鈕。未到結局時要從頭重玩，可在回憶頁選故事起點；已到結局時，主按鈕直接開始新一輪。
