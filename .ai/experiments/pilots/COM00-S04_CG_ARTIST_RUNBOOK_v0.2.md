...
> 使用 GitHub repo `TsungmingLiu/seventeen-floor-neighbor` 的 `main` branch，執行 `.ai/experiments/pilots/tasks/COM00-S04-cg-artist-v0.2.md`。先逐項確認所有指定 Markdown 真的讀到內容、所有指定 Google Drive 圖真的取得像素並可見；任何一項拿不到就立刻 BLOCKED，不要假設、不要補資料、不要生圖。只有 preflight 全部 PASS 才生成一張 candidate，且不得自行重試。
